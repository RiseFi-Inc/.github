## Description

A clear and concise description of the PR.

Use this section for review hints, explanations or discussion points/todos.

- Summary of changes
- Reasoning
- Additional context

## Screenshots

Screenshots or a screen recording of the visual changes associated with this PR.

(Feel free to delete this section for non-visual changes.)

## Docs

Add any notes that help to document the feature/changes. Doesn't need to be your best writing, just a few words and/or code snippets.

## Known TODOs

Are there any remaining chores or refactors that should be addressed in the future? If so, list them here and create the associated Linear tickets. Make a checklist; otherwise, remove this section.
